<?php
session_start(); // // mulai session supaya bisa akses data user yang login (misalnya NIS, role, dll)
...
if (isset($_POST['update'])) { // // cek apakah tombol UPDATE ditekan (form sudah dikirim)
    ...
    // UPDATE kategori = perintah untuk mengubah data di tabel kategori

    // SET = menentukan kolom yang mau diubah
    // ket_kategori='$kategori' = update nama/keterangan kategori dengan data baru dari form

    // WHERE id_kategori='$id' = kondisi wajib!
    // hanya kategori dengan id tersebut yang akan diubah
    // kalau tidak pakai WHERE → semua kategori di database bakal keubah (bahaya 💀)

    // $kategori = biasanya diambil dari input form ($_POST)
    // $id = id kategori yang mau diedit (biasanya dikirim lewat URL atau form)

    // tujuan keseluruhan:
    // mengubah nama/keterangan kategori berdasarkan id tertentu
