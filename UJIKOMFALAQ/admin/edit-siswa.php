...
if (isset($_POST['update'])) { // // cek apakah tombol UPDATE diklik (form dikirim)
    ...
    // UPDATE user = perintah untuk mengubah data di tabel user

    // SET = bagian untuk menentukan kolom mana saja yang mau diubah
    // username='$username' = update username dengan data baru dari form
    // password='$password' = update password dengan data baru
    // nis='$nis' = update NIS user
    // kelas='$kelas' = update kelas user

    // WHERE id='$id' = kondisi wajib!
    // hanya data dengan id tersebut yang akan diubah
    // kalau tidak pakai WHERE → semua data di tabel user bakal keubah (bahaya 💀)

    // $username, $password, $nis, $kelas = biasanya diambil dari $_POST (input form)

    // tujuan keseluruhan:
    // mengupdate data user berdasarkan id tertentu (biasanya dari form edit profil)
